...
CONSOL_ERROR_LOG_PATH=/bd-ogcmr/uat/cmc/cmc-core/report/cmc-health-check-report
...
## Step 2: list the reports generated
#CMC-admin obsolete
#echo "List of report generated: " >> ${LOCAL_REPORT_LIST_LOG}
#touch --date "${BATCH_DATE} 00:00:00" /tmp/cmc_start_foo
#touch --date "${BATCH_DATE} 23:59:59" /tmp/cmc_end_foo

#find ${REPORT_PATH} -newer /tmp/cmc_start_foo ! -newer /tmp/cmc_end_foo -ls | sort >> ${LOCAL_REPORT_LIST_LOG}

## Step 3: if this server is primary host, copy the consolidated information from remote server of the same site
if [ x"${CURR_HOSTNAME}" = x"${PRIMARY_HOSTNAME}" ]
then
...
## Step 4: if this server is primary host, send the consolidated information via email
${JAVA_HOME}/bin/java -Djboss.ejb.client.properties.file.path=${PROPERTIES_FILE} -Dlog4j.configuration=file:${CONFIG}/log4j_KB.properties -cp ${CLASSPATH} ${BATCH_CLASS} ${PROPERTIES_FILE} R ${IMPL_CLASS} execute "cmcLogCheckBatchDate=$BATCH_DATE"
