...
public class IAS04MReport implements IReportImplement {

    private static Log logger = LogFactory.getLog(IAS04MReport.class);
    ...
    public List<ReportResult> generateReport(AdmRptInfo_ admRptInfo, Map<String, String> paramMap) throws Exception {
        ...
        try {
            ...
            String requestDateStr = displaySdf.format(requestDate);

            String requestYearMonth = yearMonthSdf.format(requestDate);
            String startDate = "01" + requestDateStr.substring(2);

            Calendar calendar = Calendar.getInstance();
            calendar.set(new Integer(requestYearMonth.substring(0, 4)).intValue(),
                    new Integer(requestYearMonth.substring(4, 6)).intValue() - 1, 1);
            String endDate = Integer.toString(calendar.getActualMaximum(Calendar.DAY_OF_MONTH))
                    + requestDateStr.substring(2);
            ...
            parameters.put("REQUEST_MONTH", requestYearMonth);
            ...
            result.setDataStartDate(displaySdf.parse(startDate));
            result.setReportingDate(reportingDate);
            result.setOutputFileByteArray(baos.toByteArray());
            resultList.add(result);

            if (logger.isInfoEnabled())
                logger.info("IAS04MReport successful reportingDateStr[" + reportingDateStr + "], requestDate["
