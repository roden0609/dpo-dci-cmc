...
//	public static PrivateKey privateKey = null;
	...
	{
		log.debug("EIDUtils encryptToBase64 encryptedTextStr = "+encryptedTextStr);
		...
		log.debug("EIDUtils encryptToBase64 result = "+result);
		return result ;
	}
	...
	public static String genSignatureHmacSHA256(String timestamp,String nonce, String reqBody) throws Exception
	{
		...
		{
			clientSecret = KMUUtils.getPasswordFromKMU(runtimeProperty, CommonConstants.EID_CLIENT_SECRET_ID_PROPERTY_NAME,CommonConstants.EID_CLIENT_SECRET_USAGE_TYPE_PROPERTY_NAME);
		}
	...
	public static String decryptSymmetricContentKeyFromBase64Encode(String encryptedBase64RawData) throws Exception
	{
		...
		// CMC-2023-008: Renewal iAM Smart KEK cert in ESP & KMU - BEGIN
//		if(privateKey == null) {
		PrivateKey privateKey = KMUUtils.getPrivateKeyByFriendlyAlias(runtimeProperty, runtimeProperty.getProperty(CommonConstants.EID_ENC_DEC_PRIVATE_KEY_FRIENDLY_ALIAS_PROPERTY_NAME));
//		}
		if(privateKey instanceof RSAPrivateKey) {
			log.info("decryptSymmetricContentKeyFromBase64Encode - private key getModulus:" + ((RSAPrivateKey)privateKey).getModulus());
		}
		// CMC-2023-008: Renewal iAM Smart KEK cert in ESP & KMU - END
	...
	public static boolean doGetSymmetricEncryptionKey(String targetURL,String reqTimeStamp,String reqNonce)throws Exception
	{
		log.debug("doGetSymmetricEncryptionKey start"+":"+targetURL+":"+reqTimeStamp+":"+reqNonce+":"+clientID);
		...
		headerParms.put("user-agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.99 Safari/537.36");
		...
		if(returnResult !=null)
		{
			...
			if(successCode.equalsIgnoreCase(responseCode) && content != null && content.length()>0)
			{
				...
				log.info("EIDUtils response error code"+":"+responseCode);
				log.info("EIDUtils response end error msg"+":"+message);
			}
		}
	...
	public static EIDResponseBean doRequestGetNotificationIDs(String targetURL,  EServiceOpenIdsBean eSerivceOpenIdsBean)throws Exception
	{
		String reqTimeStamp = EIDUtils.doGenCurrentTimeInMilliSecond();
		String reqNonce = EIDUtils.genStateByUsingUUID();
		EIDResponseBean returnBean =  doRequestGetNotificationIDs(targetURL, reqTimeStamp, reqNonce, eSerivceOpenIdsBean);

		//If Encryption/Decryption Error Code,renew CEK and fire a request one more time.
		if(returnBean != null)
		{
			log.info("EIDUtils doRequestGetNotificationIDs returnBean.getCode()="+returnBean.getCode());
	...
	public static EIDResponseBean doRequestSwitchNotificationIDsByHKIDs(
		...
		String reqTimeStamp = EIDUtils.doGenCurrentTimeInMilliSecond();
		String reqNonce = EIDUtils.genStateByUsingUUID();
		EIDResponseBean returnBean =  doRequestSwitchNotificationIDsByHKIDs(targetURL, reqTimeStamp, reqNonce, eSerivceHkidssBean);

		//If Encryption/Decryption Error Code,renew CEK and fire a request one more time.
		if(returnBean != null) {
			log.info("EIDUtils doRequestSwitchNotificationIDsByHKIDs returnBean.getCode()="+returnBean.getCode());
			if( ENCRYPT_DECRPT_ERROR_CODE_D30001.equalsIgnoreCase(returnBean.getCode())
					||ENCRYPT_DECRPT_ERROR_CODE_D30002.equalsIgnoreCase(returnBean.getCode())
					||ENCRYPT_DECRPT_ERROR_CODE_D30003.equalsIgnoreCase(returnBean.getCode())
					||ENCRYPT_DECRPT_ERROR_CODE_D30004.equalsIgnoreCase(returnBean.getCode()))
			{
				...
				reqTimeStamp = EIDUtils.doGenCurrentTimeInMilliSecond();
				reqNonce = EIDUtils.genStateByUsingUUID();
				returnBean = doRequestSwitchNotificationIDsByHKIDs( targetURL, reqTimeStamp, reqNonce, eSerivceHkidssBean);
			}
		}
	...
	private static EIDResponseBean doRequestGetNotificationIDs(
			String targetURL,String reqTimeStamp,String reqNonce, EServiceOpenIdsBean eSerivceOpenIdsBean)throws Exception
	{
		log.info("EIDUtils doRequestGetNotificationIDs targetURL="+targetURL+", clientID="+clientID+", reqTimeStamp="+reqTimeStamp+", reqNonce="+reqNonce);
		...
		log.debug("EIDUtils doRequestGetNotificationIDs reqBodyStr="+reqBodyStr);
		...
		{
			String encryptedRequestBody = encryptToBase64(reqBodyStr,Base64.decodeBase64(symmetricEncryptionKey.getBytes("UTF-8")));
			if( encryptedRequestBody != null && encryptedRequestBody.length()>0)
			{
				...
				log.debug("EIDUtils doRequestGetNotificationIDs encrypted reqBodyStr="+reqBodyStr);
			}
	...
	private static EIDResponseBean doRequestSwitchNotificationIDsByHKIDs(
			String targetURL, String reqTimeStamp, String reqNonce, EServiceHkidsBean eServiceHkidsBean) throws Exception {
		log.info("EIDUtils doRequestSwitchNotificationIDsByHKIDs targetURL=" + targetURL + ", clientID=" + clientID
		...
		log.debug("EIDUtils doRequestSwitchNotificationIDsByHKIDs reqBodyStr="+reqBodyStr);
		...
		if (symmetricEncryptionKey != null) {
			...
			if( encryptedRequestBody != null && encryptedRequestBody.length() > 0) {
				...
				EIDEncryptContentBean.setContent(encryptedRequestBody);
				reqBodyStr =  gson.toJson(EIDEncryptContentBean);
				log.debug("EIDUtils doRequestSwitchNotificationIDsByHKIDs encrypted reqBodyStr="+reqBodyStr);
			}
	...
	public static EIDResponseBean doRequestQueryNotificationDeliveryStatus(String targetURL, TxIdBean txIdBean)throws Exception
	{
		String reqTimeStamp = EIDUtils.doGenCurrentTimeInMilliSecond();
		String reqNonce = EIDUtils.genStateByUsingUUID();
		EIDResponseBean returnBean =  doRequestQueryNotificationDeliveryStatus(targetURL, reqTimeStamp, reqNonce, txIdBean);
			...
			String targetURL,String reqTimeStamp,String reqNonce,  TxIdBean txIdBean)throws Exception
	{
		...
		log.info("EIDUtils doRequestQueryNotificationDeliveryStatus targetURL="+targetURL+", clientID="+clientID+", reqTimeStamp="+reqTimeStamp+", reqNonce="+reqNonce);
		...
		log.debug("EIDUtils doRequestQueryNotificationDeliveryStatus reqBodyStr="+reqBodyStr);
		...
		if(symmetricEncryptionKey != null)
		{
			...
			if( encryptedRequestBody != null && encryptedRequestBody.length()>0)
			{
				...
				EIDEncryptContentBean.setContent(encryptedRequestBody);
				reqBodyStr =  gson.toJson(EIDEncryptContentBean);
				log.debug("EIDUtils doRequestQueryNotificationDeliveryStatus encrypted reqBodyStr="+reqBodyStr);
			}
	...
	public static EIDResponseBean doRequestSendNotificationMessages(String targetURL, NotificationBean notificationBean)throws Exception
	{
		String reqTimeStamp = EIDUtils.doGenCurrentTimeInMilliSecond();
		String reqNonce = EIDUtils.genStateByUsingUUID();
		log.info("doRequestSendNotificationMessages pass 2, reqTimeStamp=" + reqTimeStamp +
				",reqNonce=" +  reqNonce );
		EIDResponseBean returnBean =  doRequestSendNotificationMessages(targetURL, reqTimeStamp, reqNonce,  notificationBean);
	...
	private static EIDResponseBean doRequestSendNotificationMessages(String targetURL,String reqTimeStamp,String reqNonce, 
		NotificationBean notificationBean) throws Exception {

		log.info("EIDUtils doRequestSendNotificationMessages targetURL=" + targetURL + ", clientID=" + clientID + ", reqTimeStamp=" + reqTimeStamp + ", reqNonce=" + reqNonce);
		gson = new Gson();
		if (targetURL == null || targetURL.isEmpty() ||
			clientID == null || clientID.isEmpty() ||
			reqTimeStamp == null || reqTimeStamp.isEmpty() ||
		...
		log.debug("EIDUtils doRequestSendNotificationMessages raw reqBodyStr="+reqBodyStr);
		...
		if(symmetricEncryptionKey != null)
		{
			...
			if( encryptedRequestBody != null && encryptedRequestBody.length()>0)
			{
				...
				EIDEncryptContentBean.setContent(encryptedRequestBody);
				reqBodyStr =  gson.toJson(EIDEncryptContentBean);
				log.debug("EIDUtils doRequestSendNotificationMessages encrypted reqBodyStr="+reqBodyStr);
			}
	...
	public static EIDResponseBean doRequestPushNotificationMessages(String targetURL, PushNotificationBean pushNotificationBean) throws Exception {
		String reqTimeStamp = EIDUtils.doGenCurrentTimeInMilliSecond();
		String reqNonce = EIDUtils.genStateByUsingUUID();
		log.info("doRequestPushNotificationMessages pass 2, reqTimeStamp=" + reqTimeStamp + ",reqNonce=" +  reqNonce);
		EIDResponseBean returnBean = doRequestPushNotificationMessages(targetURL, reqTimeStamp, reqNonce, pushNotificationBean);
	...
	private static EIDResponseBean doRequestPushNotificationMessages(
		String targetURL, String reqTimeStamp, String reqNonce, PushNotificationBean pushNotificationBean) throws Exception {

		log.info("doRequestPushNotificationMessages targetURL=" + targetURL + ", clientID=" + clientID + ", reqTimeStamp=" + reqTimeStamp + ", reqNonce=" + reqNonce);
		gson = new Gson();
		if (targetURL == null || targetURL.isEmpty() ||
			clientID == null || clientID.isEmpty() ||
			reqTimeStamp == null || reqTimeStamp.isEmpty() ||
	...
	public static boolean initialize(Properties inRuntimeProperty)throws Exception
	{
		...
		try{
		    if(inRuntimeProperty != null)
		  {
			...
			{
				symmetricEncryptionKeyAPIURL =  runtimeProperty.getProperty(CommonConstants.EID_SYM_ENC_KEY_REQUEST_URL_PROPERTY_NAME);
			...
			{
				clientID = runtimeProperty.getProperty(CommonConstants.EID_CLIENT_ID_PROPERTY_NAME, CommonConstants.EID_CLIENT_ID_DEFAULT_VALUE).trim();
				if(clientID == null || clientID .length()<=0)
				{
			...
			if(clientSecret == null)
			{
				clientSecret = KMUUtils.getPasswordFromKMU(runtimeProperty, runtimeProperty.getProperty(CommonConstants.EID_CLIENT_SECRET_ID_PROPERTY_NAME),runtimeProperty.getProperty(CommonConstants.EID_CLIENT_SECRET_USAGE_TYPE_PROPERTY_NAME));
		  	...
		  	{
			  revokeSymmetricEncryptionKeyAPIURL =  runtimeProperty.getProperty(CommonConstants.EID_REVOKE_SYM_ENC_KEY_REQUEST_URL_PROPERTY_NAME);
			  if(revokeSymmetricEncryptionKeyAPIURL ==null || revokeSymmetricEncryptionKeyAPIURL.length()<=0)
			  {
...
//			}
			...
			{
				String eIDProxyPortString = runtimeProperty.getProperty(CommonConstants.EID_PROXY_PORT_PROPERTY_NAME);
			...
			{
				mobileAppContextURL = runtimeProperty.getProperty(CommonConstants.EID_MOBILEAPP_CONTEXT_URL_PROPERTY_NAME);
			}
	...
	{
		log.debug("EIDUtils initForWebServiceEIDCallBack:"+inRuntimeProperty);
		if(inRuntimeProperty != null)
		{
			runtimeProperty = inRuntimeProperty;
		...
		synchronized (INITIALIZATION_MUTEX)
		{
			...
			for(int i=0; (i<4 && (getSuccess==false)); i++)
			{
				if(symmetricEncryptionKey == null || symmetricEncryptionKeyExpiryTime == null
						|| (symmetricEncryptionKeyExpiryTime != null && symmetricEncryptionKeyExpiryTime.before(currentTimestamp))
						|| forceUpdate)
				{
					getSuccess = doGetSymmetricEncryptionKey(symmetricEncryptionKeyAPIURL,(currentTimestamp.getTime()+""), reqNonce);
				}
			}
	...
	public static String hashBySHA256(String value) throws NoSuchAlgorithmException {
		return new String(EncoderUtils.base64Encode(MessageDigest.getInstance(HASH_ALGORITHM).digest(value.getBytes())));
	}
	...
	public static String openIAMSmartAppUrlFormation(String type,String ticketID)
	{
		...
		if(mobileAppContextURL != null && mobileAppContextURL.length() >0)
		{
			...
			{
				openEIDAppURL = mobileAppContextURL + "://"+mobileAppContextProfile+"?"+"ticketID="+encodeURLValue(ticketID);
			}else if(mobileAppContextEme.equalsIgnoreCase(type))
			{
				openEIDAppURL = mobileAppContextURL + "://"+mobileAppContextEme+"?"+"ticketID="+encodeURLValue(ticketID);
			}else if(mobileAppContextReauth.equalsIgnoreCase(type))
			{
				openEIDAppURL = mobileAppContextURL + "://"+mobileAppContextReauth+"?"+"ticketID="+encodeURLValue(ticketID);
			}
		}
	...
	// CMC-2023-008: Renewal iAM Smart KEK cert in ESP & KMU - BEGIN
	public static void doRevokeSymmetricEncryptionKey(String targetURL,String reqTimeStamp,String reqNonce)throws Exception
	{
	...
	public static void revokeSymmetricEncryptionKey() throws Exception
	{
		synchronized (INITIALIZATION_MUTEX)
		{
			...
			Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
			String reqNonce = genStateByUsingUUID();
			doRevokeSymmetricEncryptionKey(revokeSymmetricEncryptionKeyAPIURL,(currentTimestamp.getTime()+""), reqNonce);
